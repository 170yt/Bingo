...
public class Gamemanager {
    ...
    private static Map<UUID, ArrayList<Material>> missingMaterialsPlayer = new HashMap<>();
    ...
    public void initializeMaps() {
        ...
        if (Main.getInstance().getConfig().getBoolean("settings.isTeamGame")) {
            ...
            for (int i = 1; i <= 9; i++) {
                ...
                missingMaterialsTeam.put("team" + i, list);
            }
            Main.getBackpack().clearAllBp();
        } else {
            Bukkit.getOnlinePlayers().forEach(player -> {
                scorePlayer.put(player.getUniqueId(), 0);
                missingMaterialsPlayer.put(player.getUniqueId(), list);
            });
        }
    }
